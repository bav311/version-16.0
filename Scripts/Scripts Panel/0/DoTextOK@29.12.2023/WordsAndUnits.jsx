var ScriptDate = "29.12.2023"; // это дата выпуска скрипта, с которым работает этот набор настроек
//
// С предыдущими по дате выпуска версиями DoTextOK эти настройки работать не будут.
// 
//01.12.2022
var defis_Text = "DOI:, doi:, ГОСТ, ОСТ, ISBN, ISSN, ISMN, ORCID:"; // это строка указанных через запятую сокращений, в номере после которых числа должны быть разделены дефисом. ||  ТУ  СанПиН
suffChars = "[-—–‑:\\._\\d/\(\)a-zA-Z]+"; // эти знаки могут быть в ГОСТ, ОСТ, ISBN, ISSN, doi:
// В номерах ГОСТ, ISBN и пр. всегда будет дефис. Какой -- обычный или неразрывный -- выбирается радиокнопкой на первой вкладке  скрипта (Чистка текста)

// 27.01.2023
// ЕСЛИ В ОБРАБОТКЕ ТЕКСТА ИСПОЛЬЗУЕТСЯ ФЛАЖОК ОБРАБОТКИ МАТЕМАТИЧЕСКИХ ЗНАКОВ, ТО СТРОКИ СО СТАНДАРТАМИ DOI:, doi:, ГОСТ, ОСТ, ISBN, ISSN, ORCID НЕ ДОЛЖНЫ ПОПАДАТЬ В ЭТУ ОРАБОТКУ.


/*
DOI:    
При формировании суффикса допустимо использовать только следующие символы: a-z, A-Z, 0-9 и -._;()/
Суффикс не чувствителен к регистру символов. Например, идентификаторы 10.1006/abc и 10.1006/ABC являются одинаковыми.
*/

/*
Идеи на будущее.    
По этому сетевому адресу https://www.enago.ru/academy/isbn-doi-issn-a-quick-guide-to-publication-identifiers/
есть образец объединённого кода SICI
0095-4403 (199502/03) 21: 3 <12: WATIIB> 2.0.TX; 2-J

Тут внутри номера используются пробелы, так что если возникнет задача добавить в строку defis_Text еще сокращение SICI или SISAC, 
это просто добавлением угловых скобок в suffChars не решить, тут более изощрённый grep-запрос будет.
Но пока про SICI никто не говорил, что нужно.
...
//
// 19.01.2020
//var twoSlashesKerning = "-q300"; // зачение кернинга для двойных прямых наклонных линий (слешей). Реализуется так, что первому слешу назначается сиимв. стиль 'SlashTracking' с трекингом, определённым в этой переменной. Трекинг второго слеша не изменяется.

// 26.10.2019
// Установки шпаций при сбросе теперь устанавливаются тут. Раньше они были указаны в скрипте.
// Почему теперь 4 ?  Мне так нравится.  Я считаю, что эта шпация вместе с пространством над точкой инициала 
// и зримо отделяют инициал от фамилии и в то же время группа ИОФ не разваливается на отдельные части.
// Вы можете делать иначе.
_myASPushkinFirstSpace_selection_start = 4; // раньше было 5
_myASPushkinSecondSpace_selection_start = 4;  // раньше было 2
_myPushkinASFirstSpace_selection_start = 4;  // раньше было 2
_myPushkinASSecondSpace_selection_start = 4;  // раньше было 5
...
var splitExceptions = ["либо", "нибудь", "таки", "как", "то"]; // вторые части слов с дефисами как указатели на слова, которые не должны переноситься по этому дефису: 'кто-либо', 'когда-нибудь' -- можно запретить перенос по имеющемуся дефису.
// 12.05.2022 
// 1)  заменяется сам дефис, из обычного он становится неразрывным (0x2011).
// 2) На вкладке "Оформление текста" добавлен флажок "В двойных словах сделать неразрывным дефис перед короткими частицами", разрешающий или запрещающий использование массива splitExceptions.
// 3) значение этого флажка при сбросе и по умолчанию опреляется переменной splitExceptionsUsage_value_start: если 1, то флажок установлен, если 0, то сброшен.
var splitExceptionsUsage_value_start = 0;
...
//
var sameSpaces = true; // когда true, при выборе любой новой шпации до/после тире соседняя становится такой же
//
// Вариант межсловных и межбуквенных интервалов и масштаба глифов для кириллицы, лучше работающий с русскими текстами в сравнении со стандартным вариантом индизайна. Тот под английский язык заточен, под их короткие слова и узкие буквы.
var WordSpacing = "95/100/125";  
var LetterSpacing = "-5/0/5";
//var GlyphScaling = "98/100/102";
var GlyphScaling = "99/100/101";
///
...
var hideUserName = false; // по умолчанию false. Если true, то на флажке "Оформление сноски определяется установками абазцного стиля" не выводится информация о купившем программу
////
var sleepTime = 1000; // число миллисекунд, в течение которых на экране будет держаться сообщение о завершении работы скрипта, стандартно 2000
////
...
var flexNobreakOnly = false; // если true, то в обрабатываемом тексте все фиксированные неразрывные пробелы будут заменены на изменяемые неразрывные пробелы.
///
...
var SpaceLetterNonbreakingSpace = "(A|a|а|а,|в|и|и,|к|о|с|у|i|i,|і,|і|ў|ў,|з|я|я,|\"О|\"К|\"В|В|И|И,|К|О|С|У|А|Я|Я,|\"И|С|\"З|З|\"Ў|Ў|\"I|\"І|І|I|У|\"У|\"А|А,|А|АО|ООО)";
///
...
var myGrecDeuSpecChars = "([α-ωἀϕϖäöüÄÖÜß¼½¾⅓⅔⅛⅜⅝⅞ְֱֲֳִֵֶַָֹֺֻּֽ֑֖֛֢֣֤֥֦֧֪֚֭֮֒֓֔֕֗֘֙֜֝֞֟֠֡֨֩֫֬֯־ֿ׀ׁׂ׃ׅׄ׆ׇאﭏבגדהוזחטיךכלםמןנסעףפץצקרשתװױײ׳״יִﬞײַﬠﬡﬢﬣﬤﬥﬦﬧﬨ﬩שׁשׂשּׁשּׂאַאָאּבּגּדּהּוּזּטּיּךּכּלּמּנּסּףּפּצּקּרּשּתּוֹבֿכֿפֿ])";
...
var ButtonSize = [ 179,"2560, 174", "1600,174", "1680, 174","1920, 174", "1366,170", "1280,170" ]; // ширина кнопок ||  24/3/2020 - кнопки на 10 пк меньше по ширине
